...
命令行执行：
  ...
  python src/ai_training/train_buysell_lstm_ensemble_v2.py

或者指定数据目录：

  python src/ai_training/train_buysell_lstm_ensemble_v2.py ai_training_data/buysell_training_data
...
训练完成后，自动保存文件到：
  ...
  trained_models/buysell_ensemble_models_v2/
...
步骤2️⃣：加载模型和标准化器
  import joblib
  model = joblib.load('trained_models/buysell_ensemble_models_v2/ensemble_model.pkl')
  scaler = joblib.load('trained_models/buysell_ensemble_models_v2/scaler.pkl')
...
def load_buysell_training_data(data_dir, period='day'):
    ...
    目录结构:
    data_dir/
        buy_signal/data.json      # 买入信号
        sell_signal/data.json     # 卖出信号
        hold/data.json            # 观望持有
    """
    print("正在初始化分析器...")
    ...
    features_list = []
    labels_list = []
    signal_dirs = {'sell_signal': 0, 'hold': 1, 'buy_signal': 2}
...
def create_ensemble_model_v2(X_train, y_train, X_test, y_test, scaler):
    ...
    # 数据预处理
    print("\n📊 数据预处理:")
    print(f"  原始数据形状: {X_train.shape}  # (样本数, 60根K线, 42个特征)")
    ...
    # 准备检查点目录
    checkpoint_dir = Path('./trained_models/buysell_ensemble_models_v2/checkpoints')
    ...
    if 'history' in locals():
        plot_learning_curves(history, output_dir=Path('./trained_models/buysell_ensemble_models_v2'))
...
def main():
    print("=" * 70)
    print("买卖点信号预测 - Ensemble集成模型训练（优化版v2.0）")
    ...
    if not os.path.exists(data_dir):
        print(f"\n❌ 数据目录不存在: {data_dir}")
        print("\n📖 使用方法:")
        print("   python src/ai_training/train_buysell_lstm_ensemble_v2.py <训练目录>")
        print("\n📖 示例:")
        print("   python src/ai_training/train_buysell_lstm_ensemble_v2.py ai_training_data\\buysell_training_data")
        print("\n📁 目录结构:")
        print("   buysell_training_data/")
        print("     ├── buy_signal/      # 买入信号")
        print("     ├── sell_signal/     # 卖出信号")
        print("     └── hold/            # 观望持有")
        return
    ...
    # 8. 保存模型和元数据
    output_dir = Path('./trained_models/buysell_ensemble_models_v2')
    ...
    print("  pred = model.predict(X_new_std)  # 返回 [0:卖出, 1:观望, 2:买入]")
