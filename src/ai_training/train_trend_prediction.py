...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
目录结构：
  ai_training_data/{period}_kline_training/
    ├── up_trend/data.json         # 上涨趋势的K线数据（JSON格式）
    ├── down_trend/data.json       # 下跌趋势的K线数据
    └── sideways/data.json         # 横盘震荡的K线数据

支持的周期（period）：
  • day_kline_training      # 日K线
  • week_kline_training     # 周K线
  • 1hour_kline_training    # 1小时K线
  • 30min_kline_training    # 30分钟K线
  • 15min_kline_training    # 15分钟K线
  • 5min_kline_training     # 5分钟K线
...
要求：
  ...
  ✓ 上涨趋势：K线出现底部反弹，趋势向上，MA5>MA25
  ✓ 下跌趋势：K线出现顶部压力，趋势向下，MA5<MA25
  ✓ 横盘震荡：K线无方向，在区间内震荡，MA5接近MA25
...
命令行执行：
  ...
  python ai_training/train_trend_prediction.py ai_training_data/day_kline_training

或者其他周期：

  python ai_training/train_trend_prediction.py ai_training_data/1hour_kline_training

训练过程中会自动：
  ✓ 检测K线周期（day/1hour/30min等）
  ✓ 加载和验证K线数据
...
训练完成后，自动保存文件到：
  ...
  trained_models/trend_prediction_models_{period}/
  ├── trend_model.h5              ⭐ PC版模型（主要使用这个！）
  ├── trend_model.tflite          ⭐ 手机版模型（约250KB）
...
关键文件（必须保留）：
  • trend_model.h5       - 必须！包含LSTM模型权重
  • trend_model.tflite   - 必须！手机版模型
...
步骤2️⃣：加载模型和标准化器
  ...
  model = keras.models.load_model('trained_models/trend_prediction_models_day/trend_model.h5')
  scaler = joblib.load('trained_models/trend_prediction_models_day/scaler.pkl')
...
6. ❌ 混淆不同周期的模型
   ✅ 日K线用day_kline_training的模型，1小时用1hour的模型
...
═══════════════════════════════════════════════════════════════════════════
  ...
  自适应参数（根据数据量自动调整）：
    < 50条：   Epochs=150, Batch=8,  Dropout=0.3, EarlyStop=15
    50-200条： Epochs=200, Batch=16, Dropout=0.4, EarlyStop=20
    200-500条：Epochs=250, Batch=24, Dropout=0.4, EarlyStop=25
    > 500条：  Epochs=300, Batch=32, Dropout=0.45, EarlyStop=30
...
def load_training_data(data_dir, period='day'):
    ...
    print(f"\n总计: {len(X)} 条")
    print(f"  下跌趋势: {np.sum(y == 0)} 条")
    print(f"  横盘震荡: {np.sum(y == 1)} 条")
    print(f"  上涨趋势: {np.sum(y == 2)} 条")
...
def parse_training_path(path: str) -> dict:
    """
    从训练路径自动解析周期类型
    
    示例:
        ai_training_data/1hour_kline_training
        → period='1hour'
    
    支持的周期:
        - day_kline_training → 'day'
        - week_kline_training → 'week'
        - 5min_kline_training → '5min'
        - 15min_kline_training → '15min'
        - 30min_kline_training → '30min'
        - 1hour_kline_training → '1hour'
    """
    result = {
        'period': 'day',
        'valid': False
    ...
    for pattern, period in period_map.items():
        if pattern in path:
...
def main():
    print("=" * 70)
    print("趋势预测模型训练（LSTM - 优化版 v2.0）")
    print("基于: 时序特征 + 价格均线 + MACD + 成交量")
    print("=" * 70)
    ...
    # 2. 解析路径，获取周期信息
    path_info = parse_training_path(data_dir)
    period = path_info['period']
    
    print(f"\n🕐 检测到K线周期: {period}")
    print(f"   → 将自动从对应的数据表读取K线数据")
    
    # 验证目录是否存在
    if not os.path.exists(data_dir):
        print(f"\n❌ 数据目录不存在: {data_dir}")
        print("\n📖 使用方法:")
        print("   python ai_training/train_trend_prediction.py <训练目录>")
        print("\n📖 示例:")
        print("   python ai_training/train_trend_prediction.py ai_training_data/day_kline_training")
        print("   python ai_training/train_trend_prediction.py ai_training_data/1hour_kline_training")
        print("\n📁 目录结构:")
    ...
    # 3. 加载数据
    print("\n开始提取特征...")
    X, y = load_training_data(data_dir, period=period)
    ...
    # 9. 准备检查点目录
    output_dir = Path(f'./trained_models/trend_prediction_models_{period}')
    ...
    # 17. 保存最终模型（H5格式）
    model_path = output_dir / 'trend_model.h5'
    ...
    # 20. 转换为TFLite格式（手机版）
    print(f"\n📱 正在将模型转换为TFLite格式...")
    tflite_path = output_dir / 'trend_model.tflite'
    ...
    print("\n🎯 预测输出说明:")
    print("  0 - 下跌趋势：预期未来3-7天下跌")
    print("  1 - 横盘震荡：预期未来3-7天横盘")
    print("  2 - 上涨趋势：预期未来3-7天上涨")
    
    print("\n📁 生成的文件:")
    print(f"  保存位置: {output_dir}/")
    print(f"  1. trend_model.h5        - PC版模型")
    print(f"  2. trend_model.tflite    - 手机版模型（约250KB）")
    ...
    print("  ")
    print(f"  # 加载模型和标准化器（注意周期：{period}）")
    print(f"  model = keras.models.load_model('trained_models/trend_prediction_models_{period}/trend_model.h5')")
    print(f"  scaler = joblib.load('trained_models/trend_prediction_models_{period}/scaler.pkl')")
    print("  ")
    print("  # ========== 预测步骤（关键！顺序不能变） ==========")
    print("  # 步骤1: 提取特征")
